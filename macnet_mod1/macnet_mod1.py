...
def repr_function(output, batch, VOCAB, LABELS):
    ...
    for idx, c, q, a, o in zip(indices, story, question, answer, output):
        ...
        results.append([ c, q, a, o, str(a == o) ])
...
if __name__ == '__main__':
    ...
    log.info('dataset size: {}'.format(len(trainset)))
    log.info('dataset[:10]: {}'.format(pformat(trainset[0])))
    ...
    if 'train' in sys.argv:
        train_set = sorted(trainset, key=lambda x: -len(x.story))
        test_set  = sorted(testset, key=lambda x: -len(x.story))

        _batchop = partial(batchop, VOCAB=VOCAB, LABELS=LABELS)
        train_feed     = DataFeed(SELF_NAME, train_set, batchop=_batchop, batch_size=config.HPCONFIG.batch_size)
        test_feed      = DataFeed(SELF_NAME, test_set, batchop=_batchop, batch_size=config.HPCONFIG.batch_size)
        predictor_feed = DataFeed(SELF_NAME, test_set, batchop=_batchop, batch_size=1)

        predictor = Predictor(SELF_NAME, model=model, directory=ROOT_DIR, feed=predictor_feed, repr_function=partial(repr_function, VOCAB=VOCAB, LABELS=LABELS))
