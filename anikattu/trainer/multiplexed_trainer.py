...
class MultiplexedTrainer(object):
    ...
    def __build_stats(self):
        ...
        self.sampling_distribution_file = '{}/results/metrics/{}.{}'.format(
            self.ROOT_DIR,
            self.name,
            'sampling_distribution.pkl')
        self.sampling_distribution_records = defaultdict(list)
        try:
            with open(self.sampling_distribution_file, 'rb') as f:
                self.sampling_distribution_records.update(pickle.load(f))
        except:
            log.error('{} file don\'t exist'.format(self.sampling_distribution_file))
        ...
        for epoch in range(self.epochs):
            ...
            for j in tqdm(range(self.feed.num_batch), desc='Trainer.{}'.format(self.name)):
                ...
                input_ = self.feed.next_batch(
                    sampling_distribution = self.sampling_distribution(epoch)
                )
            ...
            for k, v in self.feed.sampling_distribution_counter.items():
                self.sampling_distribution_records[k].append(v)

            with open(self.sampling_distribution_file, 'wb') as f:
                pickle.dump(self.sampling_distribution_records, f)
                
            self.log.info(
                pformat(
                    sorted(self.feed.sampling_distribution_counter.items(),
                           key     = lambda x: x[1],
                           reverse = True)))
    ...
    def _sampling_distribution(self, epoch):
        return {k: 1 - v.accuracy[-1] for k, v in self.testers.items()}
    
    def _do_every_checkpoint(self, epoch, early_stopping=True):
        for t in self.testers.values():
