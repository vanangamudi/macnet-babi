...
def train(config, argv, name, ROOT_DIR,  model, dataset):
    ...
    train_feed     = DataFeed(name, dataset.trainset, batchop=_batchop, batch_size=config.HPCONFIG.batch_size)
    ...
    test_feed[name]      = DataFeed(name, dataset.testset, batchop=_batchop, batch_size=config.HPCONFIG.batch_size)
