...
if __name__ == '__main__':
    ...
    if 'train' in sys.argv:
        ...
        predictor = Predictor(SELF_NAME, model=model, directory=ROOT_DIR, feed=predictor_feed, repr_function=partial(repr_function, VOCAB=dataset.input_vocab, LABELS=dataset.output_vocab))
        ...
        test_feed[dataset.name]      = DataFeed(dataset.name, dataset.testset, batchop=_batchop, batch_size=config.HPCONFIG.batch_size)

        tester[dataset.name] = Tester(name  = dataset.name,
                                      config   = config,
                                      model    = model,
                                      directory = ROOT_DIR,
                                      loss_function = loss_,
                                      accuracy_function = accuracy,
                                      feed = test_feed[dataset.name],
                                      predictor=predictor)
